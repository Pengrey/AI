...
class BayesNet:
    ...
    """
    # My solution
    # Individual probability for a variable with a given val
    # It is computed summing all the jointProbs of the situations where 
    # that variable has that specific value
    def individualProb(self, var, val):
        prob = 0
        for (mothers, _) in self.dependencies[var].items():
            prob += self.jointProb(list(mothers)+[(var, val)])
        return prob
    """
